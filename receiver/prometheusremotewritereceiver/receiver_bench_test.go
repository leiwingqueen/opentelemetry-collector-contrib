...
func makeWriteV2Request(numSeries, samplesPerSeries, extraLabels int) *writev2.Request {
	...
	for i := range numSeries {
		...
		for s := range samplesPerSeries {
			samples = append(samples, writev2.Sample{Value: float64(1), Timestamp: int64(s + 1)})
		}
...
func setupMetricsReceiver2(tb testing.TB) *prometheusRemoteWriteReceiver {
	tb.Helper()
				...
				b.Run(name, func(b *testing.B) {
					...
					b.RunParallel(func(pb *testing.PB) {
						for pb.Next() {
							...
							r.Header.Set("Content-Type", fmt.Sprintf("application/x-protobuf;proto=%s", "io.prometheus.write.v2.Request"))
